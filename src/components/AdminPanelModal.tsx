...
export const AdminPanelModal: React.FC<AdminPanelModalProps> = ({
  ...
  const sanitizeFileName = (name: string) => {
    return name.replace(new RegExp('[^a-zA-Z0-9._-]', 'g'), '_');
  };
  ...
  const handleGalleryUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      try {
        ...
        const fileName = 'VEERANSH_' + Date.now() + '_' + sanitizeFileName(file.name);
  ...
  const handleVideoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const fileName = 'VEERANSH_REEL_' + Date.now() + '_' + sanitizeFileName(file.name);
  ...
  const handleSliderImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const fileName = 'VEERANSH_SLIDER_' + Date.now() + '_' + sanitizeFileName(file.name);
  ...
  const handleSliderVideoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const fileName = 'VEERANSH_SLIDER_VID_' + Date.now() + '_' + sanitizeFileName(file.name);
      const storageRef = ref(storage, `banners/slider/${fileName}`);
      const uploadTask = uploadBytesResumable(storageRef, file);
  ...
  const handleCategoryGalleryUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const fileName = 'VEERANSH_CAT_' + Date.now() + '_' + sanitizeFileName(file.name);
  ...
  const handleAdImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const fileName = 'VEERANSH_AD_' + Date.now() + '_' + sanitizeFileName(file.name);
  ...
  const handleAdVideoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const fileName = 'VEERANSH_AD_VIDEO_' + Date.now() + '_' + sanitizeFileName(file.name);
      const storageRef = ref(storage, `banners/ad/${fileName}`);
      const uploadTask = uploadBytesResumable(storageRef, file);
  ...
  const parseCsvText = (csvText: string) => {
    const lines = csvText.split(new RegExp('\\r?\\n')).filter((line) => line.trim() !== '');
    if (lines.length <= 1) return [];

    const headers = lines[0].split(',').map((h) => h.trim().replace(new RegExp('^"|"$', 'g'), ''));
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      for (let c = 0; c < rawLine.length; c++) {
        ...
        if (char === '"') {
          ...
          values.push(currentValue.trim().replace(new RegExp('^"|"$', 'g'), ''));
          currentValue = '';
        } else {
          currentValue += char;
        }
  ...
  const handleCsvFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      ...
      skipEmptyLines: true,
      complete: (results) => {
        const parsedRows: BulkPreviewRow[] = results.data.map((rowObj: any, i: number) => {
          const productName = rowObj['productName'] || rowObj['Name'] || rowObj['product_name'] || `Royal Saree #${i + 1}`;
          const category = (rowObj['category'] as CategoryName) || 'Banarasi';
          const mrp = Number(rowObj['mrp']) || 9999;
          const salePrice = Number(rowObj['salePrice']) || Number(rowObj['sale_price']) || 4999;
          const rewardPoints = Number(rowObj['rewardPoints']) || Math.floor(salePrice * 0.05) || 250;
          ...
          const washCare = rowObj['washCare'] || 'Dry Clean Only';
          const description = rowObj['description'] || 'Traditional handloom saree with Silk Mark certificate.';
          const enableAR = String(rowObj['enableAR'] || '').toUpperCase() === 'TRUE' || String(rowObj['enableAR']) === '1';

          const rawImageFileNames = rowObj['imageFileNames'] || rowObj['images'] || '';
          const imageFileNames = String(rawImageFileNames).split('|').map((img: string) => img.trim()).filter(Boolean);
          const videoFileName = rowObj['videoFileName']?.trim() || undefined;
          ...
          return {
            ...
            description,
            enableAR,
            imageFileNames: imageFileNames.length > 0 ? imageFileNames : [`saree_${i + 1}.jpg`],
            videoFileName,
          };
        });

        console.log("PapaParse CSV parsed successfully:", parsedRows.length, "rows");
        setBulkPreviewRows(parsedRows);
        setBulkTotalCsvRows(parsedRows.length);
      },
      error: (error: any) => {
  ...
  const handleStartMegaUpload = async () => {
    ...
    for (let i = 0; i < bulkPreviewRows.length; i += CHUNK_SIZE) {
      ...
      console.log('[BulkUpload] Starting batch chunk #' + chunkIndex + ' with ' + chunk.length + ' products...');
      ...
      for (const row of chunk) {
        try {
          ...
          for (const fileName of row.imageFileNames) {
            ...
            if (fileObj) {
              try {
                const storageRef = ref(storage, 'bulk-sale-2026/' + Date.now() + '_' + sanitizeFileName(fileObj.name));
          ...
          if (row.videoFileName) {
            ...
            if (videoObj) {
              try {
                const videoRef = ref(storage, 'bulk-sale-videos/' + Date.now() + '_' + sanitizeFileName(videoObj.name));
          ...
          const finalImages = uploadedImageUrls.length > 0 ? uploadedImageUrls : ['https://images.unsplash.com/photo-1610030469983-98e550d6193c?auto=format&fit=crop&w=800&q=80'];
          ...
          const docRef = doc(collection(db, 'products'), prodId);
          batch.set(docRef, newProd);
          chunkProducts.push(newProd);
      ...
      try {
        await batch.commit();
        console.log('[BulkUpload] Successfully committed batch chunk #' + chunkIndex + ' (' + chunkProducts.length + ' products written to Firestore).');
      } catch (batchErr) {
        console.error('[BulkUpload] FAILED batch commit for chunk #' + chunkIndex + ':', batchErr);
      }
      ...
      for (const p of chunkProducts) {
        try {
          ...
          console.warn('Backend API add product offline sync', apiErr);
        }
      }

      newCreatedProducts.push(...chunkProducts);
      await new Promise((resolve) => setTimeout(resolve, 200));
    }

    setBulkIsUploading(false);
    onUpdateProducts([...newCreatedProducts, ...products]);
    console.log(`[BulkUpload] Mega Upload Completed! Total Successfully Uploaded: ${newCreatedProducts.length}`);
    alert(`${newCreatedProducts.length} Sarees Uploaded Successfully for Sale with PapaParse & Chunked Batch Persistence!`);
  };
  ...
  const handlePageContentChange = (val: string) => {
    setEditablePages((prev) =>
      prev.map((p) => (p.slug === activePageSlug ? { ...p, content: val } : p))
    );
  };

  const handleRewardRateChange = (val: number) => {
    setRewardSettings((prev) => ({ ...prev, pointToRupeeRate: val }));
  };
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                    ...
                    <div className="bg-white border-2 border-amber-300/80 rounded-2xl p-3.5 shadow-sm">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="w-6 h-6 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold text-xs">Rs</div>
                      </div>
                      <p className="text-base font-extrabold text-pink-950">Rs.{totalSalesAmount.toLocaleString('en-IN')}</p>
                        ...
                        <div key={c.id} className="border border-amber-300 rounded-2xl p-3 bg-amber-50/30 flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-[11px] text-slate-600 mt-1 font-semibold">
                              {c.discountType === 'PERCENT' ? `${c.discountValue}% OFF` : `Flat Rs.${c.discountValue} OFF`} (Min Rs.{c.minOrderAmount})
                            </p>
                          </div>
                          <button
              ...
              {activeTab === 'PAGES' && (
                <div className="space-y-4">
                  ...
                  <div className="bg-white border-2 border-amber-300/80 rounded-3xl p-5 shadow-sm space-y-3">
                    <div className="flex items-center justify-between">
                      <h4 className="font-serif-royal text-base font-bold text-pink-950">Edit Policy Page</h4>
                      <span className="text-xs text-slate-500">Active Slug: {activePageSlug}</span>
                    </div>

                    <textarea
                      rows={8}
                      value={editablePages.find((p) => p.slug === activePageSlug)?.content || ''}
                    ...
                    <button
                      onClick={() => {
                        const p = editablePages.find((item) => item.slug === activePageSlug);
                        if (p) handleSaveEditablePage(p.content);
                      }}
                  ...
                  <div className="bg-white border-2 border-amber-300/80 rounded-3xl p-5 shadow-sm space-y-3 max-w-xl">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        onChange={(e) => handleRewardRateChange(Number(e.target.value))}
                        className="w-24 px-3 py-1.5 rounded-xl border border-amber-300 text-xs font-bold text-pink-950"
                      />
                      <span className="text-xs font-bold text-pink-900">Rs. (Rupees)</span>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleManualWalletCredit} className="bg-white border-2 border-amber-300/80 rounded-3xl p-5 shadow-sm space-y-3 max-w-xl">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <input
                        ...
                        placeholder="User Phone or Email"
                        required
                        className="px-3 py-2 rounded-xl border border-amber-300 text-xs"
                      />
                      <input
        ...
        <div className="fixed inset-0 z-50 bg-pink-950/80 backdrop-blur-sm flex items-center justify-center p-2 overflow-y-auto">
          <div className="bg-[#FDFBF7] w-full max-w-2xl rounded-3xl p-5 shadow-2xl border-2 border-amber-500 my-auto relative max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveProduct} className="space-y-4 text-xs font-medium text-pink-950">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                  <label className="block font-bold mb-1">MRP Price (Rs.)</label>
                  <input
                    type="number"
                    value={prodFormMrp}
                ...
                <div>
                  <label className="block font-bold mb-1">Sale Price (Rs.)</label>
                  <input
                    type="number"
                    value={prodFormSale}
        ...
        <div className="fixed inset-0 z-50 bg-pink-950/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-xl rounded-3xl p-6 shadow-2xl border-2 border-amber-400 my-auto text-pink-950 relative space-y-4">
              ...
              <tbody className="divide-y divide-slate-100">
                {selectedOrderForInvoice.items.map((i, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="p-2 text-right font-bold">Rs.{i.product.salePrice.toLocaleString('en-IN')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="text-xs font-bold space-y-1 text-right border-t border-slate-200 pt-2">
              <p>Subtotal: Rs.{selectedOrderForInvoice.subtotal.toLocaleString('en-IN')}</p>
              <p className="text-emerald-700">Discount: -Rs.{selectedOrderForInvoice.discount}</p>
              <p className="text-base text-pink-900 font-extrabold pt-1">Total Paid: Rs.{selectedOrderForInvoice.totalAmount.toLocaleString('en-IN')}</p>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-pink-950/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl border-2 border-amber-400 my-auto text-pink-950 space-y-4">
            ...
            <div className="text-xs space-y-2">
              ...
              <p><strong>Lifetime Spend:</strong> Rs.{selectedCustomerHistory.totalSpend.toLocaleString('en-IN')}</p>
