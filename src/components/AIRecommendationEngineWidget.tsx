...
import { Sparkles, Sliders, CheckCircle2, Heart, ArrowRight } from 'lucide-react';
...
export const AIRecommendationEngineWidget: React.FC<AIRecommendationEngineWidgetProps> = ({
  ...
  const fetchQuickRecommendations = async () => {
    ...
    try {
      ...
      // 100% SAFE FIX - Vercel par 404 HTML aayega to JSON Parse Crash nahi hoga
      if (!res.ok) throw new Error(`API 404 - AI Local Fallback`);
                    ...
                    <div>
                      ...
                      <div className="flex items-baseline gap-1 mt-0.5">
                        <span className="text-xs font-extrabold text-[#9D174D]">
                          ₹{(Number(prod.salePrice?? 0)).toLocaleString('en-IN')}
                        </span>
